...
- The reference workbench keeps Pi's built-in `bash` for host inspection, authoring, testing, and CLI work. Session
  ingestion audits its command/result digests, but arbitrary filesystem side effects are not scientific artifacts.
  Any process whose outputs support a scientific claim must be declared as `compute.run` and resolved through
  `bio_query` or a declared operation. Inline media in a tool result may still be retained as a `session_image` audit
  artifact; that is distinct from a run-linked declared output.
- The Pi extension emits just-in-time `compute.run` guidance for prompts likely to require a plot, figure, external
  scientific runtime, or workflow. This is an agent-context reminder, not shell interception. Pi's `user_bash` hook
  is only for human-entered `!` and `!!` commands; do not conflate it with assistant `bash` tool calls.
