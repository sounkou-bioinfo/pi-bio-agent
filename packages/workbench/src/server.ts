...
export async function startWorkbenchServer(workspaceArg = ".pi/workbench", portArg = "8787", groundingModule?: string): Promise<ServerType> {
  ...
  const addons = [createArtifactWorkbenchAddon(workspace)];
  ...
  if (clinicalBinding) {
    ...
    addons.unshift(createClinicalWorkbenchAddon(clinicalOptions), createClinicalReanalysisWorkbenchAddon(clinicalOptions));
  }
