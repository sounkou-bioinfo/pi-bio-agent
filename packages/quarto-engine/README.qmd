...
Quarto's `include`, `echo`, and `output` cell options are honored. This lets a
document keep host-composition code executable but hidden while rendering the
scientific command and result. Markdown and figures render directly; text and
JSON outputs are collapsed so logs and structured payloads do not dominate the
document.
