...
# NOTE: `notes sync/report` is the pre-temporal-store path (file notes -> a separate graph.duckdb via kg-sync).
# Memory now lives as append-only, as-of observations in the ONE store (agent:memory: in bio_observations);
# this command is superseded once the memory tools are rewired to that store.
