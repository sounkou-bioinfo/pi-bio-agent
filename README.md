...
completion exposes invokable Pi commands. Artifacts renders declared
CAS-backed figures and reports; starting against the clinical example
explicitly adds its Evidence and Reanalysis regression panes. Immutable
case revisions and their CAS-addressed narrative, family, indexed
variant, and optional prior-assessment assets are registered through the
workbench CLI or HTTP API before a real case run. Tool payloads and raw
lifecycle deltas are collapsible diagnostics. The live
