...
`scripts/live-multi-agent.ts` is a **pattern** (not a unit test — non-deterministic, spawns real LLM agents). It
...
Run it with: `npm run pattern:live-multi-agent` (requires the `pi` CLI + a configured provider).
