---
title: "Host-neutral SDK embedding"
engine: pi-bio
format: gfm
---

An embedded host supplies SQL, CAS, compute, policy, capability receipts, and an application-owned resolver through
the public `pi-bio-agent` package. The SDK records the same run evidence used by CLI and Pi adapters.
...
This establishes surface parity through the public SDK. Host functions are capabilities supplied again at replay;
their identities and receipts are recorded, but executable closures are not serialized into replay data.
