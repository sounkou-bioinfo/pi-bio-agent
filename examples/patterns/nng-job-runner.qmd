---
title: "Remote worker status in the observation ledger"
engine: pi-bio
format: gfm
---

A coordinator owns the ledger and records a queued job. A separate process reports `running` and `succeeded` through
the same observation API over DuckNNG RPC. The status slot is data; no second job-state protocol is introduced.
...
This proves that a remote worker can use the ordinary temporal ledger contract. It does not itself prove durable
leases, retry, result collection, or scheduler integration; those belong to the queue worker and async-runner tests.
