---
title: "Cross-process mutable SQL over DuckNNG RPC"
engine: pi-bio
format: gfm
---

The server explicitly registers DuckNNG's exec method. Separate clients then perform native DuckDB `INSERT`,
`UPDATE`, `DELETE`, and `ON CONFLICT` statements against one server-owned table.
...
The pattern proves native mutable SQL over an explicitly admitted RPC service. It does not make arbitrary remote SQL
safe; the host still owns method registration, SQL authorization, credentials, and transport policy.
