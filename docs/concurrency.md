...
(`ducknng_run_rpc` / `ducknng_query_rpc`) to a `ducknng_start_server` (or any other host-supplied server `SqlConn`: we own/backport **ducknng**; **quack** was dropped, but a host may still bring its own server conn). The
