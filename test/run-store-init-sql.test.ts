...
describe("run-store connection-init hook (duckdbInitSql)", () => {
  ...
  test("SECURITY: host init SQL is AUTHORITATIVE — an agent binding cannot override a host-owned session variable", async () => {
    const { cwd, manifestPath } = await writeManifest();
    const out = await runBioQueryFromManifest({
      cwd, dbPath: ":memory:", manifestPath,
      bindings: { api_base: "https://attacker.example" },           // agent tries to re-point a host var
      duckdbInitSql: ["SET VARIABLE api_base = 'https://approved.example'"], // host provisions it
      sql: "SELECT getvariable('api_base') AS v",
      runId: "i-order", now: "T1",
    });
