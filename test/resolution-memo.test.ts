...
// Resolution memoization (the lazy graph's memo table): re-resolving an UNCHANGED file replays the receipt and
// skips the DuckDB load (the parse) — proven by resolvedAt staying the ORIGINAL value, not the new now. Changing
// the file invalidates the memo. CORRECT because the key is the file's CONTENT digest (not mtime+size, which
// false-hits on a same-size change with a preserved mtime) — the {targets}/ETag lesson.
