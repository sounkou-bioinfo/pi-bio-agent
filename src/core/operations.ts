...
export async function runQuery(
  ...
  opts: {
    ...
    /** Host-declared manifest directory for resolving relative resource paths during replay. */
    manifestBaseDir?: string;
  },
): Promise<{ result: OperationResult; run: BioRunRecord; receipts: ResolutionReceipt[] }> {
  const { resources, runId, now, params = [], signal, cas, remoteCacheScope, manifestBaseDir } = opts;
  ...
  try {
    for (const rid of resources) {
      receipts.push(await registry.resolveResource(rid, { conn, now, signal, cas, remoteCacheScope, manifestBaseDir }));
    }
...
export async function runOperation(
  registry: BioRegistry,
  conn: SqlConn,
  opts: { operationId: string; resources?: string[]; params?: readonly unknown[]; runId: string; now: string; signal?: AbortSignal; cas?: CasStore; remoteCacheScope?: string; manifestBaseDir?: string },
): Promise<{ result: OperationResult; run: BioRunRecord; receipts: ResolutionReceipt[] }> {
  ...
  return runQuery(registry, conn, {
    sql: op.sql.sqlTemplate, resources, runId: opts.runId, now: opts.now, params: opts.params, signal: opts.signal, cas: opts.cas, remoteCacheScope: opts.remoteCacheScope, manifestBaseDir: opts.manifestBaseDir,
