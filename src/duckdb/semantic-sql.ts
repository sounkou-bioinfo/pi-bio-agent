...
/** Upstream contract used for compatibility testing. This is the set of concrete CREATE VIEW names generated by
 * `src/semsql/builder/sql_schema/semsql.sql` at the pinned commit; abstract LinkML classes are intentionally absent. */
export const SEMANTIC_SQL_UPSTREAM_COMPATIBILITY = {
  repository: "https://github.com/INCATools/semantic-sql",
  commit: "83503077e867419c18a211d97105d8ead554e947",
  concreteViews: [
    "node_to_node_statement", "node_to_value_statement", "rdf_type_statement", "rdfs_subclass_of_statement",
    "rdfs_subclass_of_named_statement", "rdfs_subproperty_of_statement", "rdfs_label_statement",
    "rdfs_domain_statement", "rdfs_range_statement", "rdf_first_statement", "rdf_rest_statement",
    "rdf_rest_transitive_statement", "rdf_list_member_statement", "node", "node_identifier", "blank_node",
    "rdf_list_node", "iri_node", "class_node", "property_node", "named_individual_node", "count_of_predicates",
    "count_of_instantiated_classes", "count_of_subclasses", "ontology_node", "object_property_node",
    "transitive_property_node", "symmetric_property_node", "reflexive_property_node", "irreflexive_property_node",
    "asymmetric_property_node", "annotation_property_node", "deprecated_node", "owl_imports_statement",
    "owl_inverse_of_statement", "owl_complement_of_statement", "owl_equivalent_class_statement",
    "owl_same_as_statement", "owl_disjoint_class_statement", "owl_reified_axiom", "owl_axiom",
    "owl_axiom_annotation", "owl_some_values_from", "owl_all_values_from", "owl_has_value", "owl_has_self",
    "owl_subclass_of_some_values_from", "owl_equivalent_to_intersection_member", "has_text_definition_statement",
    "has_exact_synonym_statement", "has_broad_synonym_statement", "has_narrow_synonym_statement",
    "has_related_synonym_statement", "has_synonym_statement", "has_exact_match_statement",
    "has_broad_match_statement", "has_narrow_match_statement", "has_related_match_statement",
    "has_match_statement", "has_dbxref_statement", "has_mapping_statement", "contributor", "creator", "orcid",
    "axiom_dbxref_annotation", "ontology_status_statement", "trailing_whitespace_problem",
    "property_used_with_datatype_values_and_objects", "node_with_two_labels_problem", "all_problems", "edge",
    "subgraph_edge_by_ancestor", "subgraph_edge_by_descendant", "subgraph_edge_by_ancestor_or_descendant",
    "subgraph_edge_by_parent", "subgraph_edge_by_child", "subgraph_edge_by_self", "entailed_subclass_of_edge",
    "entailed_type_edge", "entailed_edge_cycle", "entailed_edge_same_predicate_cycle", "transitive_edge",
  ],
} as const;
...
function targets(spec: SemanticSqlSourceSpec): Required<SemanticSqlSourceViewTargets> {
  const out: Required<SemanticSqlSourceViewTargets> = {
    ...
    synonymsTable: spec.targets?.synonymsTable ?? "has_synonym_statement",
    ...
    mappingsTable: spec.targets?.mappingsTable ?? "has_mapping_statement",
    ...
    transitiveEdgeTable: spec.targets?.transitiveEdgeTable ?? "transitive_edge",
...
FROM ${qident(t.edgeTable)} e`,
    ...
    `CREATE OR REPLACE VIEW ${qident(t.transitiveEdgeTable)} AS
WITH RECURSIVE closure(subject, predicate, object, depth, visited) AS (
  SELECT subject, predicate, object, 1, [subject, object]
  FROM ${qident(t.edgeTable)}
  WHERE subject IS NOT NULL AND predicate IS NOT NULL AND object IS NOT NULL
  UNION ALL
  SELECT e.subject, e.predicate, c.object, c.depth + 1, list_prepend(e.subject, c.visited)
  FROM ${qident(t.edgeTable)} e
  JOIN closure c ON e.object = c.subject AND e.predicate = c.predicate
  WHERE NOT list_contains(c.visited, e.subject)
)
SELECT subject, predicate, object, min(depth) AS depth
FROM closure
...
export async function materializeSemanticSqlSourceViews(conn: SqlConn, spec: SemanticSqlSourceSpec): Promise<MaterializedSemanticSqlViews> {
  ...
  return {
    ...
    transitiveEdgeTable: t.transitiveEdgeTable,
