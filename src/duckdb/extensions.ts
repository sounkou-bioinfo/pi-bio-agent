...
export const bioDuckDbExtensions: DuckDbExtensionDescriptor[] = [
  ...
  },
  {
    name: "ducknng",
    source: "community",
    purpose: "Cross-process/cross-machine transport over NNG scalability protocols (pub/sub, push/pull, survey, bus, pair) plus framed Arrow-IPC RPC. ducknng_run_rpc(url, sql, tls) executes a SQL string on a server running native DuckDB, so UPDATE/DELETE/ON CONFLICT work there; ducknng_query_rpc(url, sql, tls) reads rows back. Exec is opt-in: the server must ducknng_register_exec_method(...), with per-method auth, peer/IP allowlists, and ducknng-native TLS/mTLS handles backed by generated self-signed material, in-memory PEM, or files. This is how multiple agent processes share one live mutable DB: the server owns it, agents talk RPC, and no client opens the file. The same extension also ships ducknng_ncurl / ducknng_ncurl_table for SQL-native HTTP. It complements CAS-of-bytes: ducknng RPC is live mutable state, CAS is durable immutable sharing. The community build is signed; a source build is unsigned, so the host sets allow_unsigned_extensions=true in duckdbConfig at DB open to load it.",
