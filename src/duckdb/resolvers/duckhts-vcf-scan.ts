...
//
// Rules (deliberate): no bespoke VCF parser fallback; no INSTALL and no network here — the host provisions
// the extension and this only LOADs it, failing closed if it is unavailable. The 163 VCF-parsing
// reimplementations in ClawBio collapse to this one resolver behind the BioResolverImpl contract.
...
const EXTRACT_SQL = (table: string) => `CREATE TABLE ${table} AS
...
export const duckhtsVcfScanResolver: BioResolverImpl = async (resource, ctx) => {
  ...
  // Fail closed if duckhts is not loadable. No INSTALL, no network — the host must provision the extension.
  try {
