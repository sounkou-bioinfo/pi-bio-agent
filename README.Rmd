...
# custom knitr engine: run a LIVE pi agent session and weave its reply into the README
knitr::knit_engines$set(pi = function(options) {
  ...
  out <- tryCatch(
    ...
    error = function(e) paste("[pi unavailable:", conditionMessage(e), "]"))
  knitr::engine_output(options,
    code = paste0('pi -e extensions/pi-coding-agent/index.ts -p "', prompt, '"'), out = out)
})

# custom knitr engine: run the pi-bio-agent CLI (displayed as `pi-bio-agent ...`, executed via the built bin)
knitr::knit_engines$set(biocli = function(options) {
  ...
  knitr::engine_output(options, code = paste("pi-bio-agent", args), out = out)
})
```
...
```{pi}
Over examples/variant-counts/manifest.json: how many variants of each consequence are there? Give the counts as a short markdown table and one sentence.
```
