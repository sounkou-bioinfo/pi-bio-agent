...
keeps it to ~20s, not minutes — the reads are ~2s; the rest was model latency). Unlike the zero-config block above,
this one is **host-provisioned**: it needs `npm run provision:duckhts` and a host that permits `duckhts`'s HTTP
range reads (DuckDB-native egress — the host's boundary, not a library default):
