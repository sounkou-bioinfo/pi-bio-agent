...
# We control the render: JSON output gets a ```json fence for syntax highlighting.
knitr::knit_engines$set(biocli = function(options) {
  args <- paste(options$code, collapse = " ")
  out <- suppressWarnings(system(paste("node dist/cli/bin.js", args), intern = TRUE))
...
```{biocli}
query examples/connectors/uniprot.json --db :memory: --init-sql "INSTALL ducknng FROM community; LOAD ducknng; SET VARIABLE tls = ducknng_tls_config_from_files(NULL, '/etc/ssl/certs/ca-certificates.crt', '', 1)" --bindings '{"uniprot_acc":"P04637"}' --sql "SELECT primaryAccession, uniProtkbId, sequence.length AS aa FROM uniprot_entry"
```
