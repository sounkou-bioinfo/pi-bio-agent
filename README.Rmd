...
substrate, never computed by it**. Reproducibility (`reproduce()`), long-running jobs, and this governance loop all
...
Using the manifest examples/connectors/clinvar-region.json, resolve its 'clinvar' resource (a ClinVar VCF region read over HTTP via duckhts). Discover the schema, note that INFO_CLNSIG is an array, then write and run your OWN SQL to count variants by clinical significance (UNNEST + GROUP BY, top 8). Show the SQL you ran and the result table.
```
