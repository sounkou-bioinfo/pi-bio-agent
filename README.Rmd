...
`duckhts.read_bcf` are conveniences over it. **The format surface is open** — a new format is a new DuckDB
...
PLINK ([`plinking_duck`](https://duckdb.org/community_extensions/extensions/plinking_duck)) — and not even bio:
HTML/XML/web ([`duckdb_webbed`](https://github.com/teaguesterling/duckdb_webbed)),
Markdown ([`duckdb_markdown`](https://github.com/teaguesterling/duckdb_markdown)), source-code ASTs across 27
languages ([`sitting_duck`](https://github.com/teaguesterling/sitting_duck)), git history
([`duck_tails`](https://github.com/teaguesterling/duck_tails)). You bring the format; DuckDB's full reach and its
fast-growing community-extension ecosystem *are* the data layer — source **and** its edit history as SQL is,
incidentally, a *codebase knowledge graph over edits*, the same substrate pointed at itself.
...
discovered, not invented. The overlap only confirms the substrate is real. The difference is what it runs *on*:
...
substrate those features are approximations of. (A UI is just a thin client over the CLI/SDK.)

## Demonstration

Every block below is runnable and its output is **real** — the same literate-programming discipline that keeps
[the example docs](examples/) from drifting (`npm run check` fails if a recorded run goes stale).

**The agent speaks.** Point a live Pi agent at a manifest and ask in plain English — it does schema discovery,
**writes its own read-only SQL** (we never hand it the query), runs it through the substrate, and answers. A real
transcript, produced live when this README is rendered:
...
`UNNEST` + `GROUP BY`. This is a **live agent run**, produced when this README is rendered. Unlike the zero-config
block above, it is **host-provisioned**: it needs `npm run provision:duckhts` and a host that permits `duckhts`'s
HTTP range reads (DuckDB-native egress — the host's boundary, not a library default):
...
`.pi/bio-agent/runs/<runId>/run.json` (spec, status, timestamps, event log) alongside a `receipts.json` of
resolution receipts and a `result.json` of rows. [`run-ledger`](examples/run-ledger/manifest.json) reads those
back with DuckDB `read_json`, so the substrate's **own provenance is queryable with the same SQL it uses for
data** — no opaque runtime to introspect. This is the live run graph produced *by this very render*:
...
bespoke. Store-logged runs, jobs, and coloc results also fold into the `bio_observations` ledger — so the whole
workbench view is composed, not coded.
...
egress. For the agent to resolve a connector *itself* with no host provisioning, use the `http.get` form
([`uniprot-http.json`](examples/connectors/uniprot-http.json)) under the networked entrypoint — the agent fetches
the JSON, discovers the schema, and writes the extraction. Same connector, two levers: `http.get` is the fetch the
host binds; `ncurl_table` is pure SQL the host provisions.
...
(`process.compute`) and *distributed* jobs (`JobDispatch` over an NNG pool / SLURM / Modal) are wired by the host
in its own code: a `ProcessRunner` injected into the run request enables `process.compute`; a `JobDispatch`
injected into `ledgerJobRunner` drives distributed jobs. The bundled `pi-bio-agent` CLI injects neither, so a
compute/networked manifest **fails closed** there — powerful effects are granted deliberately, never ambiently.
So reproducible artifacts, connected databases, on-demand compute, and grounded review run on **your** infra,
provider-agnostic. Each [example](examples/) carries a recorded, verified run; see [what the substrate closes
