...
knitr::knit_engines$set(pi = function(options) {
  ...
  out <- tryCatch(
    system2("pi", c("-e", "extensions/pi-coding-agent/index.ts", "-p", "--no-session", prompt),
...
TP53 locus, not the whole file), *discovers the 45-column schema*, finds `INFO_CLNSIG` is an array, and writes its
**own** `UNNEST` + `GROUP BY`. A real run (captured — this one takes ~2–3 min, so it's not re-run every render):
