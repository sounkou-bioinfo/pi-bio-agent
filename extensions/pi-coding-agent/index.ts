...
const BIO_ORIENTATION = [
  ...
  "  binds the built-in resolvers `duckdb.file_scan` and `duckhts.read_bcf`; the injected-fetch network",
  "  (`ncurl_table`/`http.get`) and out-of-process `process.compute` are HOST capabilities that fail closed unless",
  "  the operator granted them. (Caveat: a file_scan/read_bcf `path` or SQL that hits a REMOTE URI can still reach",
  "  the network via DuckDB/httpfs — that egress is the host's sandbox to police, not a library gate.)",
